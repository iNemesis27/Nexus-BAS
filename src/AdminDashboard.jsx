...
const MOCK_USERS = [
  ...
  { id: 7, name: "Morgan Hale", email: "morgan@nexusbas.com", role: "manager", projects: [1, 3], joined: "2026-02-12", status: "active" },
  { id: 8, name: "Taylor Brooks", email: "taylor@westhouston.com", role: "viewer", projects: [1], joined: "2026-03-08", status: "active" },
];
...
const Badge = ({ label, color, small }) => (
  <span style={{ display: "inline-flex", alignItems: "center", padding: small ? "2px 6px" : "3px 8px", background: color + "18", border: `1px solid ${color}44`, borderRadius: 3, fontSize: small ? 9 : 10, color, fontFamily: C.font, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.06em" }}>
    {label}
  </span>
);
...
const Btn = ({ children, onClick, variant = "primary", small }) => {
  ...
  return (
    <button onClick={onClick} style={{ background: v.bg, border: `1px solid ${v.border}`, color: v.color, borderRadius: 4, padding: small ? "4px 10px" : "7px 16px", fontFamily: C.font, fontSize: small ? 9 : 10, fontWeight: 600, cursor: "pointer", letterSpacing: "0.08em" }}>
      {children}
    </button>
  );
};
...
const OverviewTab = ({ projects, users, requests }) => {
  const stats = [
    { label: "Active Projects", value: projects.filter((p) => p.status === "active").length, color: C.green, icon: "HEX" },
    { label: "Total Clients", value: users.filter((u) => u.role === "client").length, color: C.cyan, icon: "CLI" },
    { label: "Staff Members", value: users.filter((u) => u.role === "staff" || u.role === "manager").length, color: C.purple, icon: "OPS" },
    { label: "Open Requests", value: requests.filter((r) => r.status === "open").length, color: C.amber, icon: "REQ" },
  ];
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 10 }}>
        {stats.map((s) => (
          <div key={s.label} style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 6, padding: "14px 16px", borderTop: `2px solid ${s.color}` }}>
            <div style={{ fontSize: 14, color: s.color, fontFamily: C.font, fontWeight: 700 }}>{s.icon}</div>
        ...
        <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 6, padding: "14px 16px" }}>
          ...
          {projects.filter((p) => p.status === "active").map((p) => (
            <div key={p.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 0", borderBottom: `1px solid ${C.border}` }}>
              ...
              <Badge label={p.client || "Unassigned"} color={C.green} small />
            </div>
          ))}
        </div>
        <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 6, padding: "14px 16px" }}>
...
const ProjectsTab = ({ projects, setProjects, users }) => {
  ...
  const [form, setForm] = useState({ name: "", type: "Residential", city: "", client: "", staff: "", floors: 1, devices: 0, status: "pending" });
  const staffList = users.filter((u) => u.role === "staff" || u.role === "manager");
  ...
  const openNew = () => {
    setForm({ name: "", type: "Residential", city: "", client: "", staff: "", floors: 1, devices: 0, status: "pending" });
    setEditingId(null);
    setShowNew(true);
  };
  ...
  const save = () => {
    ...
    if (editingId) {
      setProjects((prev) => prev.map((p) => p.id === editingId ? { ...form, id: editingId, floors: Number(form.floors), devices: Number(form.devices) } : p));
    } else {
      setProjects((prev) => [...prev, { ...form, id: Date.now(), floors: Number(form.floors), devices: Number(form.devices) }]);
    }
    setEditingId(null);
    setShowNew(false);
  };
  ...
  const Input = ({ label, field, type = "text" }) => (
    <div>
      ...
      <input type={type} value={form[field] ?? ""} onChange={(e) => setForm((p) => ({ ...p, [field]: e.target.value }))} style={{ width: "100%", background: C.panel, border: `1px solid ${C.border2}`, borderRadius: 4, padding: "8px 10px", color: C.text, fontFamily: C.font, fontSize: 11, outline: "none", boxSizing: "border-box" }} />
    </div>
  );

  const SelectF = ({ label, field, options }) => (
    <div>
      <div style={{ fontSize: 9, color: C.text3, fontFamily: C.font, marginBottom: 5, letterSpacing: "0.1em" }}>{label.toUpperCase()}</div>
      <select value={form[field] ?? ""} onChange={(e) => setForm((p) => ({ ...p, [field]: e.target.value }))} style={{ width: "100%", background: C.panel, border: `1px solid ${C.border2}`, borderRadius: 4, padding: "8px 10px", color: C.text, fontFamily: C.font, fontSize: 11, outline: "none", cursor: "pointer" }}>
        {options.map((o) => <option key={o.value} value={o.value}>{o.label}</option>)}
      </select>
    </div>
  );
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: 16 }}>
        <button onClick={openNew} style={{ background: C.cyan + "20", border: `1px solid ${C.cyan}44`, color: C.cyan, fontFamily: C.font, fontSize: 10, fontWeight: 700, padding: "7px 18px", borderRadius: 4, cursor: "pointer", letterSpacing: "0.08em" }}>
          + NEW PROJECT
        </button>
      </div>

      {isFormOpen && (
        <div style={{ background: C.surface, border: `1px solid ${editingId ? C.amber + "44" : C.cyan + "44"}`, borderRadius: 6, padding: 20, marginBottom: 20 }}>
          <div style={{ fontSize: 10, color: editingId ? C.amber : C.cyan, fontFamily: C.font, fontWeight: 700, marginBottom: 16, letterSpacing: "0.1em" }}>
            {editingId ? `EDITING: ${form.name}` : "NEW PROJECT"}
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12, marginBottom: 12 }}>
            <Input label="Building Name" field="name" />
            <Input label="City" field="city" />
            <SelectF label="Type" field="type" options={["Residential", "Commercial", "Mixed Use", "Industrial"].map((t) => ({ value: t, label: t }))} />
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr 1fr", gap: 12, marginBottom: 16 }}>
            <SelectF label="Client" field="client" options={[{ value: "", label: "Unassigned" }, ...clientList.map((u) => ({ value: u.name, label: u.name }))]} />
            <SelectF label="Staff" field="staff" options={[{ value: "", label: "Unassigned" }, ...staffList.map((u) => ({ value: u.name, label: u.name }))]} />
            <SelectF label="Status" field="status" options={["active", "pending", "inactive"].map((s) => ({ value: s, label: s.charAt(0).toUpperCase() + s.slice(1) }))} />
            <Input label="Floors" field="floors" type="number" />
            <Input label="Devices" field="devices" type="number" />
          </div>
          <div style={{ display: "flex", gap: 10 }}>
            <button onClick={save} style={{ background: editingId ? C.amber + "20" : C.cyan + "20", border: `1px solid ${editingId ? C.amber + "44" : C.cyan + "44"}`, color: editingId ? C.amber : C.cyan, fontFamily: C.font, fontSize: 10, fontWeight: 700, padding: "8px 20px", borderRadius: 4, cursor: "pointer", letterSpacing: "0.08em" }}>
              {editingId ? "SAVE CHANGES" : "CREATE PROJECT"}
            </button>
            {editingId && (
              <button onClick={() => { if (window.confirm("Delete this project?")) { deleteProject(editingId); setEditingId(null); } }} style={{ background: C.red + "18", border: `1px solid ${C.red}44`, color: C.red, fontFamily: C.font, fontSize: 10, fontWeight: 700, padding: "8px 16px", borderRadius: 4, cursor: "pointer" }}>
                DELETE
              </button>
            )}
            <button onClick={() => { setEditingId(null); setShowNew(false); }} style={{ background: "transparent", border: `1px solid ${C.border2}`, color: C.text2, fontFamily: C.font, fontSize: 10, padding: "8px 16px", borderRadius: 4, cursor: "pointer" }}>
              CANCEL
            </button>
          </div>
        </div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        {projects.map((p) => (
          <div key={p.id} style={{ background: C.surface, border: `1px solid ${editingId === p.id ? C.amber + "66" : C.border}`, borderRadius: 6, padding: "14px 18px", display: "flex", alignItems: "center", gap: 16, borderLeft: `3px solid ${p.status === "active" ? C.green : C.amber}` }}>
            <div style={{ fontSize: 18, color: p.status === "active" ? C.green : C.amber, fontFamily: C.font }}>HEX</div>
            <div style={{ flex: 1 }}>
              ...
              <div style={{ display: "flex", gap: 20, fontSize: 10, color: C.text2, fontFamily: C.font, flexWrap: "wrap" }}>
                <span>{p.city}</span>
            ...
            <button onClick={() => openEdit(p)} style={{ background: C.amber + "18", border: `1px solid ${C.amber}44`, color: C.amber, fontFamily: C.font, fontSize: 9, fontWeight: 700, padding: "6px 14px", borderRadius: 4, cursor: "pointer", letterSpacing: "0.08em" }}>
              EDIT
            </button>
          </div>
        ))}
...
const UsersTab = ({ users, setUsers, projects }) => {
  ...
  const toggleProject = (userId, projectId) => {
    setUsers((prev) => prev.map((u) => {
      ...
      if (u.projects[0] === "all") return u;
      const has = u.projects.includes(projectId);
      return { ...u, projects: has ? u.projects.filter((p) => p !== projectId) : [...u.projects, projectId] };
    }));
  };
  ...
  return (
    <div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(5,1fr)", gap: 8, marginBottom: 20 }}>
        {["admin", "manager", "staff", "client", "viewer"].map((role) => (
          <div key={role} style={{ background: C.surface, border: `1px solid ${ROLES[role]}44`, borderRadius: 6, padding: "10px 14px", textAlign: "center" }}>
            <div style={{ fontSize: 18, fontWeight: 900, color: ROLES[role], fontFamily: C.font }}>{users.filter((u) => u.role === role).length}</div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        {users.map((u) => (
          <div key={u.id} style={{ background: C.surface, border: `1px solid ${editingId === u.id ? C.cyan + "66" : C.border}`, borderRadius: 6, overflow: "hidden" }}>
            <div style={{ padding: "12px 16px", display: "flex", alignItems: "center", gap: 14 }}>
              ...
              {u.role !== "admin" && (
                <select value={u.role} onChange={(e) => updateRole(u.id, e.target.value)} style={{ background: C.panel, border: `1px solid ${C.border2}`, borderRadius: 4, padding: "5px 8px", color: C.text, fontFamily: C.font, fontSize: 10, outline: "none", cursor: "pointer" }}>
                  {["manager", "staff", "client", "viewer"].map((r) => <option key={r} value={r}>{r.charAt(0).toUpperCase() + r.slice(1)}</option>)}
                </select>
              )}
              <button onClick={() => setEditingId(editingId === u.id ? null : u.id)} style={{ background: C.cyan + "15", border: `1px solid ${C.cyan}33`, color: C.cyan, fontFamily: C.font, fontSize: 9, fontWeight: 700, padding: "5px 12px", borderRadius: 4, cursor: "pointer", letterSpacing: "0.06em" }}>
                {editingId === u.id ? "DONE" : "PROJECTS"}
              </button>
            </div>

            {editingId === u.id && u.projects[0] !== "all" && (
              <div style={{ borderTop: `1px solid ${C.border}`, padding: "12px 16px", background: C.panel }}>
                <div style={{ fontSize: 9, color: C.cyan, fontFamily: C.font, fontWeight: 700, letterSpacing: "0.12em", marginBottom: 10 }}>ASSIGN TO PROJECTS</div>
                <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                  {projects.map((p) => {
                    const assigned = u.projects.includes(p.id);
                    return (
                      <button key={p.id} onClick={() => toggleProject(u.id, p.id)} style={{ padding: "5px 12px", borderRadius: 4, background: assigned ? C.cyan + "20" : "transparent", border: `1px solid ${assigned ? C.cyan : C.border2}`, color: assigned ? C.cyan : C.text2, fontFamily: C.font, fontSize: 10, cursor: "pointer", fontWeight: assigned ? 700 : 400 }}>
                        {assigned ? "OK " : ""}{p.name}
                      </button>
                    );
                  })}
                </div>
                <div style={{ fontSize: 9, color: C.text3, fontFamily: C.font, marginTop: 10 }}>
                  Currently assigned to: {u.projects.length > 0 ? projects.filter((p) => u.projects.includes(p.id)).map((p) => p.name).join(", ") : "No projects"}
                </div>
              </div>
            )}
            {editingId === u.id && u.projects[0] === "all" && (
              <div style={{ borderTop: `1px solid ${C.border}`, padding: "10px 16px", background: C.panel }}>
                <div style={{ fontSize: 10, color: C.purple, fontFamily: C.font }}>Admin has access to all projects</div>
              </div>
            )}
          </div>
        ))}
        ...
        {filtered.map((r) => (
          <div key={r.id} style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 6, padding: "14px 18px", borderLeft: `3px solid ${STATUS_C[r.status]}` }}>
            ...
            <div style={{ display: "flex", gap: 10, alignItems: "center", marginTop: 10, flexWrap: "wrap" }}>
              ...
              <Select value={r.assignee || ""} onChange={(v) => assign(r.id, v || null)} options={[{ value: "", label: "Unassigned" }, ...staffList.map((u) => ({ value: u.name, label: u.name }))]} />
              <span style={{ fontSize: 9, color: C.text3, fontFamily: C.font, marginLeft: 8 }}>STATUS:</span>
              <Select value={r.status} onChange={(v) => setStatus(r.id, v)} options={["open", "assigned", "in-progress", "resolved", "closed"].map((s) => ({ value: s, label: s.charAt(0).toUpperCase() + s.slice(1) }))} />
            </div>
          </div>
        ))}
      </div>
...
export default function AdminDashboard() {
  ...
  const TABS = [
    ...
    { id: "users", label: "USERS & ROLES", icon: "USR" },
    { id: "requests", label: "REQUESTS", icon: "REQ" },
  ];
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", height: 48 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              <div style={{ width: 28, height: 28, background: C.purple + "20", border: `1px solid ${C.purple}44`, borderRadius: 4, display: "flex", alignItems: "center", justifyContent: "center" }}>
                <span style={{ color: C.purple, fontSize: 10, fontFamily: C.font, fontWeight: 700 }}>ADM</span>
              </div>
              <div>
